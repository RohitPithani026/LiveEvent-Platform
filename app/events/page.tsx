...
export default function EventsPage() {
    ...
    const filteredEvents = events.filter((event) => {
        ...
        switch (categoryFilter) {
            case "today":
                matchesCategory = eventDate.getTime() === today.getTime()
                break
            case "tomorrow":
                matchesCategory = eventDate.getTime() === tomorrow.getTime()
                break
            case "next7":
                matchesCategory = eventDate > tomorrow && eventDate <= next7
                break
            case "completed":
                matchesCategory = eventDate < today
                break
            default:
                matchesCategory = true // Show all if no filter or "all"
        }
