...
const grpcClient = __importStar(require("@grpc/grpc-js"));
const PROTO_DIR = path_1.default.join(__dirname, "..", "proto");
...
(0, room_manager_1.setInteractionEventType)(InteractionEventType);
...
server.addService(eventProto.event.EventService.service, {
    ...
    StartEvent: async (call, callback) => {
        try {
            ...
            let event;
            try {
                event = await prisma_1.prisma.event.findUnique({
                    where: { id: event_id },
                });
            }
            catch (dbError) {
            ...
            try {
                room_manager_1.roomManager.emitRoomEvent(event_id, {
                    type: "EVENT_STARTED",
                    userId: host_id,
...
server.addService(roomProto.room.RoomService.service, {
    JoinRoom: async (call, callback) => {
        try {
            ...
            const success = room_manager_1.roomManager.joinRoom(event_id, user_id, is_host || false);
            if (!success && is_host) {
                // If host join failed, check if this user is already the host
                const room = room_manager_1.roomManager.getRoom(event_id);
            ...
            if (eventExists) {
                try {
                    await prisma_1.prisma.participant.upsert({
                        where: {
                            userId_eventId: {
                                userId: user_id,
                                eventId: event_id,
    ...
    LeaveRoom: async (call, callback) => {
        try {
            ...
            room_manager_1.roomManager.leaveRoom(event_id, user_id);
            callback(null, { ok: true });
        }
        catch (error) {
    ...
    UpdateMediaState: async (call, callback) => {
        try {
            const { event_id, has_video, has_audio, has_screen } = call.request;
            room_manager_1.roomManager.updateMediaState(event_id, has_video, has_audio, has_screen);
            callback(null, { ok: true });
        }
        catch (error) {
    ...
    StreamRoomEvents: (call) => {
        const { event_id } = call.request;
        // Subscribe to room events
        room_manager_1.roomManager.subscribeToRoomEvents(event_id, call);
        // Handle client disconnect
        call.on("cancelled", () => {
            room_manager_1.roomManager.unsubscribeFromRoomEvents(event_id, call);
        });
        call.on("end", () => {
            room_manager_1.roomManager.unsubscribeFromRoomEvents(event_id, call);
        });
...
server.addService(interactionProto.interaction.InteractionService.service, {
    SendMessage: async (call, callback) => {
        try {
            ...
            room_manager_1.roomManager.emitInteractionEvent(event_id, {
                type: "MESSAGE",
                userId: user_id,
    ...
    CreateQuestion: async (call, callback) => {
        try {
            ...
            const qna = await prisma_1.prisma.qnA.create({
                data: {
                    eventId: event_id,
                    userId: user_id,
                    question,
    ...
    ApproveQuestion: async (call, callback) => {
        try {
            const { question_id, event_id } = call.request;
            // Update question to approved
            const qna = await prisma_1.prisma.qnA.update({
    ...
    SendPollResponse: async (call, callback) => {
        try {
    ...
    SendQuizResponse: async (call, callback) => {
        try {
            ...
            // Update score
            const score = await prisma_1.prisma.score.upsert({
                where: {
                    userId_eventId: {
                        userId: user_id,
                        eventId: event_id,
    ...
    StreamInteractionEvents: (call) => {
        const { event_id } = call.request;
        room_manager_1.roomManager.subscribeToInteractionEvents(event_id, call);
        call.on("cancelled", () => {
            room_manager_1.roomManager.unsubscribeFromInteractionEvents(event_id, call);
        });
        call.on("end", () => {
            room_manager_1.roomManager.unsubscribeFromInteractionEvents(event_id, call);
        });
...
function createGrpcClient() {
    const client = new grpcClient.Client(`localhost:${GRPC_PORT}`, grpcClient.credentials.createInsecure());
    return client;
}
// HTTP endpoints
app.post("/api/events/:id/start", async (req, res) => {
    try {
        ...
        const client = new eventProto.event.EventService(`localhost:${GRPC_PORT}`, grpcClient.credentials.createInsecure());
        client.StartEvent({ event_id: id, host_id: payload.userId }, (error, response) => {
            if (error) {
...
app.post("/api/events/:id/end", async (req, res) => {
    try {
        ...
        const client = new eventProto.event.EventService(`localhost:${GRPC_PORT}`, grpcClient.credentials.createInsecure());
        client.EndEvent({ event_id: id, host_id: payload.userId }, (error, response) => {
            if (error) {
